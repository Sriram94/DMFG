...
default_env_args = dict(tiger_step_recover=-0.021, deer_attacked=-0.1)
...
def raw_env(map_size=default_map_size, max_cycles=max_cycles_default, minimap_mode=minimap_mode_default, **env_args):
    return _parallel_env_wrapper(parallel_env(map_size, max_cycles, minimap_mode, **env_args))
...
def get_config(map_size, minimap_mode, tiger_step_recover, deer_attacked):
    ...
    attack_deer_reward = 0.2
    cfg.add_reward_rule(gw.Event(a, 'attack', c), receiver=a, value=attack_deer_reward)
    
    
    # tigers get large reward when they attack a deer simultaneously
