...
class DeepQNetwork:
        ...
        with tf.variable_scope(self.name + 'Value'): 
            ...
            with tf.variable_scope(self.name + 'eval_net_1'):
                ...
                with tf.variable_scope(self.name + 'l_h2'):
                    wh2 = tf.get_variable('w_h2', [n_l1, n_l1], initializer=w_initializer, collections=c_names)
                    bh2 = tf.get_variable('b_h2', [1, n_l1], initializer=b_initializer, collections=c_names)
            ...
            with tf.variable_scope('target_net_1', reuse = tf.AUTO_REUSE):
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() <= self.epsilon:
            actions_value = self.sess.run(self.predict, feed_dict={self.s: observation})
