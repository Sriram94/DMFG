...
from ValueFunction import NeuralNetworkBased, DMFGQL
...
def run_epoch(envt,
    ...
    list_tmp = []
    for i in range(4461): 
        list_tmp.append(1/4461)
    for i in range(num_agents):
        agent_meanfield[i] = list_tmp

    s = 0 
    while True:
        try:
            current_requests = next(request_generator)
            mean_field = envt.get_meanfield(current_requests)
        ...
        if (is_training):
            if s == 1: 
                for i in range(num_agents):
                    value_function[i].updatemeanfield([previous_experience[i]], mean_field)
                
                print("meanfields updated")
        ...
        for i in range(num_agents):
            agent_experience[i] = Experience(agents[i], feasible_actions_all_agents[i], mean_field, envt.current_time, len(current_requests))
            previous_experience = agent_experience.copy()
             
        
        
        for i in range(num_agents):
            agent_meanfield[i] = value_function[i].get_meanfield([agent_experience[i]])
        

        for i in range(num_agents):
            agent_experience[i] = Experience(agents[i], feasible_actions_all_agents[i], agent_meanfield[i], envt.current_time, len(current_requests))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m2', '--modellocationmeanfield', type=str)
    ...
    for i in range(args.numagents): 
        if args.modellocation:
            ...
            modellocationmeanfield = './modelsmeanfield/'+str(i)+args.modellocationmeanfield
            value_function[i] = DMFGQL(envt, log_dir=LOG_DIR, load_model_loc=modellocation, load_model_loc2=modellocationmeanfield)

        else:
            value_function[i] = DMFGQL(envt, log_dir=LOG_DIR, load_model_loc=args.modellocation, load_model_loc2=args.modellocationmeanfield)
                ...
                if (isinstance(value_function[0], NeuralNetworkBased)):
                    if (test_score > max_test_score or (envt.num_days_trained % SAVE_FREQ) == (SAVE_FREQ - 1)):
                        for i in range(num_agents):
                            ...
                            value_function[i].model2.save('./modelsmeanfield/'+str(i)+'{}_{}agent_{}capacity_{}delay_{}interval_{}_{}.h5'.format(type(value_function[i]).__name__, args.numagents, args.capacity, args.pickupdelay, args.decisioninterval, envt.num_days_trained, test_score))
                            print("Saved")
