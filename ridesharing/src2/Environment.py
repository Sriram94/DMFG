...
class NYEnvironment(Environment):
    ...
    def get_meanfield(self, current_requests):

        
        mean_field = []
        source_list = []
        totalrequests = len(current_requests)
        for i in range(len(current_requests)):
            source_list.append(current_requests[i].pickup)

        for i in range(self.NUM_LOCATIONS): 
            sumrequests = 0
            for j in range(len(source_list)): 
                if i == source_list[j]:
                    sumrequests = sumrequests + 1

            ratio = sumrequests/totalrequests
            mean_field.append(ratio)
