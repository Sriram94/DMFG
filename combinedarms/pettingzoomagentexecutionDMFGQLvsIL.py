        ...
        for step in range(max_cycles):
            for agent in parallel_env.agents:
                ...
                if team == 0: 
                    ...
                    action = RL_dqn_melee[agent].choose_action(agent_observation)
                
                elif team == 3: 
                    action = RL_dqn_ranged[agent].choose_action(agent_observation)
...
if __name__ == "__main__":
    ...
    for agent in parallel_env.agents:
        ...
        if "blue" in agent:
            if "mele" in agent:
                new_name = agent
                RL_dqn_melee[agent] = DeepQNetwork(9,5915,sess, e_greedy=1, name = new_name, e_greedy_increment=None, execution=True)
                
                RL_dqn_melee[agent].restore_model("./"+agent+"/dqnmodel.ckpt")


            else:
                new_name = agent
                RL_dqn_ranged[agent] = DeepQNetwork(25,8619,sess, e_greedy=1, name = new_name, e_greedy_increment=None, execution=True)

                RL_dqn_ranged[agent].restore_model("./"+agent+"/dqnmodel.ckpt")
