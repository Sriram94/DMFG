...
class DMFGQL:
    def __init__(
        ...
        self.memory = np.zeros((self.memory_size, n_features * 2 + self.n_meanaction * 2 + 2))
    ...
    def _build_net(self):
        ...
        self.mean = tf.placeholder(tf.float32, [None, self.n_meanaction], name='a_1')  
        self.newmean = tf.placeholder(tf.float32, [None, self.n_meanaction], name='new_a1')  
        ...
        with tf.variable_scope(self.name + 'Value'): 
            ...
            with tf.variable_scope( self.name + 'eval_net_1'):
                ...
                with tf.variable_scope(self.name + 'l_3'):
                    w3 = tf.get_variable('w_3', [self.n_meanaction, n_l1], initializer=w_initializer, collections=c_names)
    ...
    def _build_net2(self):
        ...
        self.mean_action = tf.placeholder(tf.float32, [None, self.n_meanaction], name='meanaction')  
        self.target_mean = tf.placeholder(tf.float32, [None, self.n_meanaction], name='target_mean')  
        ...
        with tf.variable_scope(self.name + 'meanfieldnetwork'): 
            ...
            with tf.variable_scope(self.name + 'meanaction_net_1'):
                ...
                with tf.variable_scope(self.name + 'meanactionl_3'):
                    w3 = tf.get_variable('meanactionw_3', [self.n_meanaction, n_l1], initializer=w_initializer, collections=c_names)
                ...
                with tf.variable_scope(self.name + 'meanactionl2'):
                    w2 = tf.get_variable('meanactionw2', [n_l1, self.n_meanaction], initializer=w_initializer, collections=c_names)
                    b2 = tf.get_variable('meanactionb2', [1, self.n_meanaction], initializer=b_initializer, collections=c_names)
    ...
    def learn(self):
        ...
        q_next, q_eval = self.sess.run(
            ...
            feed_dict={
                ...
                self.mean: batch_memory[:, self.n_features:self.n_features + self.n_meanaction],
                self.newmean: batch_memory[:, self.n_features+self.n_meanaction:self.n_features+self.n_meanaction+self.n_meanaction]
                })

        q_target = q_eval.copy()

        batch_index = np.arange(self.batch_size, dtype=np.int32)
        eval_act_index = batch_memory[:, self.n_features+self.n_meanaction+self.n_meanaction ].astype(int)
        reward = batch_memory[:, self.n_features+self.n_meanaction+self.n_meanaction+1]
